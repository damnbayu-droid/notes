...
export function AuthPage({ onSignIn, onSignUp, onResetPassword, onGoogleSignIn, onBack }: AuthPageProps) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-purple-50 flex items-center justify-center p-4">
      <AuthNotification />
      {/* Background decorations - marked as aria-hidden for screen readers */}
      ...
      <button
        onClick={onBack}
        className="absolute top-4 left-4 z-50 px-4 py-2 bg-white/50 hover:bg-white/80 backdrop-blur-md rounded-full text-sm font-medium text-gray-700 transition-all hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-violet-500"

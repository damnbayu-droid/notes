...
export function CreateNoteButton({ onClick }: CreateNoteButtonProps) {
  ...
  return (
    <Card
      className="group relative overflow-hidden cursor-pointer border-dashed border-2 border-gray-300 bg-gray-50/50 hover:bg-white hover:border-violet-400 transition-all duration-300"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      onClick={onClick}
    >
      <div className="p-6 flex flex-col items-center justify-center min-h-[200px] text-center">
        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-lg transition-all duration-300 ${
          isHovered ? 'scale-110 shadow-violet-300' : ''
        }`}>
